...
interface RevenueLeakageResult {
  ...
  totalLeakageOpportunities: number;
  leakageOpportunities: {
    unbilledMedications?: any[];
    unbilledServices?: any[];
  };
...
export default function RevenueLeakageAnalysis() {
  ...
  const [leakageResult, setLeakageResult] = useState<RevenueLeakageResult | null>(null);
  ...
  const startRevenueLeakageAnalysis = async () => {
    ...
    try {
      const analysisResponse = await ApiService.startRevenueLeakageAnalysis(billDocumentId, medicalDocumentIds);
      console.log('💰 Revenue Leakage - Analysis response:', analysisResponse);
      ...
      console.log('💰 Revenue Leakage - Job ID:', jobId);
      setAnalysisProgress(25);

      const interval = setInterval(async () => {
        try {
          const statusResponse = await ApiService.getAnalysisStatus(jobId);
          console.log('💰 Revenue Leakage - Status response:', statusResponse);
          
          if (statusResponse.success && statusResponse.data) {
            const status = statusResponse.data.status;
            console.log('💰 Revenue Leakage - Job status:', status);
            ...
            if (status === 'completed') {
              ...
              console.log('💰 Revenue Leakage - Fetching result for jobId:', jobId);
              const resultResponse = await ApiService.getAnalysisResult(jobId);
              console.log('💰 Revenue Leakage - Result response:', resultResponse);
              
              const resultData = resultResponse.success && resultResponse.data ? resultResponse.data.result : null;
              console.log('💰 Revenue Leakage - Result data:', resultData);
              
              if (resultData) {
                setLeakageResult(resultData);
                setAnalysisProgress(100);
                setIsAnalyzing(false);
                toast.success('Revenue leakage analysis completed!');
              } else {
                console.error('💰 Revenue Leakage - No result data found');
    ...
    <div className="space-y-8">
      <div className="text-center">
        <h1 className="text-3xl font-bold text-gray-900 mb-4 flex items-center justify-center">
          <CurrencyDollarIcon className="h-9 w-9 mr-3 text-blue-600" />
          Revenue Leakage Analysis
        </h1>
        <p className="text-gray-600 max-w-2xl mx-auto">
          Identify medications and services documented in medical records but not billed - recover lost revenue opportunities.
        </p>
      </div>
      ...
      <div className="text-center">
        <motion.button
          onClick={startRevenueLeakageAnalysis}
        ...
        >
          {isAnalyzing ? (
            ...
            <>
              ...
              Start Revenue Leakage Analysis
            </>
          )}
        </motion.button>
      ...
      {isAnalyzing && (
        <div className="max-w-md mx-auto">
          ...
          <p className="text-center text-sm text-gray-600 mt-2">
            {analysisProgress < 30 ? 'Initializing analysis...' : analysisProgress < 60 ? 'Reviewing medical records...' : analysisProgress < 90 ? 'Identifying unbilled items...' : 'Finalizing results...'}
          </p>
        </div>
      )}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl shadow-lg p-8 border border-gray-200">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-gray-900 flex items-center">
              ...
              Revenue Leakage Results
            </h2>
            <div className="text-sm text-gray-500">{new Date(leakageResult.timestamp).toLocaleString()}</div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className={`rounded-lg p-6 border ${getImpactColor(leakageResult.revenueImpactLevel)}`}>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium">Revenue Impact</p>
                  <p className="text-2xl font-bold capitalize">{leakageResult.revenueImpactLevel || 'Unknown'}</p>
                </div>
            ...
            <div className="bg-blue-50 rounded-lg p-6 border border-blue-200">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-blue-600 text-sm font-medium">Recovery Opportunities</p>
                  <p className="text-2xl font-bold text-blue-700">{leakageResult.totalLeakageOpportunities || 0}</p>
                </div>
          ...
          <div className="space-y-6">
            <div className="bg-blue-50 rounded-lg p-6 border border-blue-200">
              <h3 className="text-lg font-semibold text-blue-800 mb-4 flex items-center">
                ...
                Unbilled Medications (Revenue Recovery)
              </h3>
              <div className="space-y-3">
                {(leakageResult.leakageOpportunities?.unbilledMedications?.length || 0) > 0 ? (
                  leakageResult.leakageOpportunities?.unbilledMedications?.map((item: any, index: number) => (
                    <div key={index} className="text-sm text-gray-600 flex items-start bg-white p-3 rounded border border-blue-200">
                      ...
                      <div>
                        ...
                        {typeof item === 'object' && item.found_in && Array.isArray(item.found_in) && <div className="text-xs text-blue-600 mt-1">Found in: {item.found_in.join(', ')}</div>}
                      </div>
                    </div>
                  ))
                ) : (
                  <div className="text-sm text-gray-500 italic">No unbilled medications found</div>
                )}
              </div>
            </div>

            {leakageResult.recommendations && leakageResult.recommendations.length > 0 && (
              <div className="bg-blue-50 rounded-lg p-6 border border-blue-200">
                <h3 className="text-lg font-semibold text-blue-800 mb-4">Revenue Recovery Actions</h3>
                <ul className="space-y-2">
                  {leakageResult.recommendations.map((action: string, index: number) => (
            ...
            <div className="bg-gray-50 rounded-lg p-6 border border-gray-200">
              ...
              <div className="space-y-3 text-sm text-gray-700">
                <p><strong className="text-blue-600">Revenue Leakage Focus:</strong> This analysis identifies medical services and medications that were provided and documented but were not included in the billing - representing lost revenue opportunities.</p>
                <p><strong>Impact Level {leakageResult.revenueImpactLevel}:</strong> {leakageResult.revenueImpactLevel === 'HIGH' && 'Significant revenue recovery potential. Immediate billing corrections recommended.'} {leakageResult.revenueImpactLevel === 'MEDIUM' && 'Moderate revenue recovery opportunities identified.'} {leakageResult.revenueImpactLevel === 'LOW' && 'Minor unbilled items found.'}</p>
                <p><strong>Action:</strong> Review each identified item and add appropriate charges to recover lost revenue while ensuring compliance with billing regulations.</p>
              </div>
            </div>
          </div>
        </motion.div>
      ...
      {showDocumentViewer && selectedDocumentForViewing && (
        <DocumentViewer
          ...
          analysisData={{
            analysisTimestamp: leakageResult?.timestamp,
            analysisSuccessful: true,
            medicalAnalysis: selectedDocumentForViewing.type === 'bill'
              ? `# Hospital Bill Document\n\nThis is the billing document being analyzed for revenue leakage.\n\n## Analysis Status\n${leakageResult ? '✓ Completed' : 'Pending'}\n\n## Revenue Impact Level\n${leakageResult ? leakageResult.revenueImpactLevel : 'N/A'}`
              : `# Medical Record Document\n\nThis is a medical record being reviewed for unbilled services.\n\n## Analysis Status\n${leakageResult ? '✓ Completed' : 'Pending'}`
          }}
          onClose={() => {
